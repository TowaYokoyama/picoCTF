...
例えば Python + Flask だと：
<h1>Hello {{ name }}</h1>
この {{ name }} の部分が プログラム的に評価される部分。
サーバー側で：
render_template("index.html", name="畑山")
↓
最終的にブラウザに送られるのは：
<h1>Hello 畑山</h1>
...
全体像（まずは超ざっくり）
この1行はこういうことをしている：
Jinja2 → Python内部 → import os → ls実行 → 結果表示
1️⃣ request
Flaskアプリでは request というオブジェクトがテンプレート内で使える。
これは「今のHTTPリクエスト情報」を持ってるオブジェクト。
2️⃣ .application
request.application
→ その Flask アプリ本体にアクセスしている。
3️⃣ .__globals__
ここが超重要。
Pythonでは：
関数やアプリは
